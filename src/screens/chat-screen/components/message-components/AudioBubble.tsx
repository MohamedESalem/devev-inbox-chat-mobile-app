...
export const AudioBubblePlayer = React.memo((props: AudioPlayerProps) => {
  ...
  const audioPlayBackStatus: Callback = useCallback(
    ({ data }) => {
  ...
  const sliderProps = useMemo(
    () => ({
      ...
      filledTrackColor: variant === MESSAGE_VARIANTS.USER ? 'bg-white' : 'bg-brand-700',
      knobStyle: variant === MESSAGE_VARIANTS.USER ? 'border-brand-300' : 'border-brand-700',
      manualSeekTo,
      currentPosition,
      totalDuration,
      pauseAudio,
